...
class SettingsActivity : AppCompatPreferenceActivity() {
	...
	/**
	 * This fragment shows info only.
	 */
	@TargetApi(Build.VERSION_CODES.HONEYCOMB)
	class InfoPreferenceFragment : PreferenceFragment() {
		private var devOptionsEnabled = false
		private var lastPreferenceClick = 0L
		private var preferenceClicks = 0
		private var currentToast: Toast? = null
		private var devOptions: Preference? = null
		...
		override fun onCreate(savedInstanceState: Bundle?) {
			...
			devOptions = findPreference("devOptions")

			devOptions?.setOnPreferenceClickListener {
				activity.startActivity(Intent(activity as Context, DeveloperOptionsActivity::class.java))
				true
			}

			preferenceScreen.removePreference(devOptions)

			findPreference("license").setOnPreferenceClickListener {
				if(!devOptionsEnabled) {
					if (lastPreferenceClick < System.currentTimeMillis() - 1000)
						preferenceClicks = 0
					preferenceClicks++
